(() => {
  ...
  const WIDTH = 1080;
  const HEIGHT = 1920;
  const BASE_LOOP_MS = 12000;
  const TARGET_FRAME_MS = 1000 / 30;
  const FRAME_TOLERANCE_MS = 1.5;
  const TAU = Math.PI * 2;
  const searchParams = new URLSearchParams(window.location.search);
  const requestedSpeed = Number(searchParams.get("speed"));
  const SPEED_MULTIPLIER = searchParams.has("speed") && Number.isFinite(requestedSpeed)
    ? Math.max(1, Math.min(4, requestedSpeed))
    : 2;
  const LOOP_MS = BASE_LOOP_MS / SPEED_MULTIPLIER;

  document.documentElement.dataset.xfgVersion = "10";
  document.documentElement.dataset.xfgSpeed = String(SPEED_MULTIPLIER);
  ...
  mask.src = "./xfg-tiktok-lime-glow-mask.png?v=10";

  window.XFGOverlay = Object.freeze({
    width: WIDTH,
    height: HEIGHT,
    loopDurationMs: LOOP_MS,
    baseLoopDurationMs: BASE_LOOP_MS,
